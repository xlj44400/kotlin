...
class ScriptDefinitionsManager(private val project: Project) : LazyScriptDefinitionProvider() {
    ...
    override fun findDefinition(fileName: String): ScriptDefinition? {
        ...
        if (ScratchFileService.getInstance().findFile(
        ...
        val definition =
            if (ScratchFileService.getInstance().findFile(
                ...
                super.findDefinition(fileName) ?: return null
            } else {
    ...
    return templateClassNames.mapNotNull { templateClassName ->
        try {
            ...
            if (thirdPartyPlugin != null) {
                LOG.error(PluginException("[kts] cannot load script definition class $templateClassName", e, thirdPartyPlugin))
            } else {
