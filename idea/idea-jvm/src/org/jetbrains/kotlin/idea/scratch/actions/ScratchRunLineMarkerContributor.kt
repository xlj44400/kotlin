...
class ScratchRunLineMarkerContributor : RunLineMarkerContributor() {
    override fun getInfo(element: PsiElement): Info? {
        ...
        val scripInitializerExpression = element.getParentOfType<KtScriptInitializer>(true)?.body
        if (scripInitializerExpression != null) {
            if (scripInitializerExpression.findDescendantOfType<LeafPsiElement>() == element) {
                return isLastExecutedExpression(element)
            }
            return null
        }
