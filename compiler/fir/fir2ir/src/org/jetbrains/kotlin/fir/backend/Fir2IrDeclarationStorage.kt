        ...
        return when (val firDeclaration = firVariableSymbol.fir) {
            is FirValueParameter -> {
                val irDeclaration = localStorage.getParameter(firDeclaration)
                    ?: throw IllegalArgumentException("Cannot find parameter ${firDeclaration.render()} in local storage")
                irSymbolTable.referenceValueParameter(irDeclaration.descriptor)
            }
            is FirVariable -> {
                val irDeclaration = localStorage.getVariable(firDeclaration)
                    ?: throw IllegalArgumentException("Cannot find variable ${firDeclaration.render()} in local storage")
